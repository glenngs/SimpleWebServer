...
public abstract class HttpRequest {
	...
	public static HttpRequest read(InputStream inputStream) throws Exception {
		...
		String methodCall = tokenizer.nextToken();// We will fill this object
													// with the data from input
		...
		try {
			...
			// We will convert both the strings to lower case to be able to
			// search later
			...
			while (!line.equals("")) {
				...
				// Lets break the line into two part with first space as a
				// separator
				...
				if (index > 0 && index < line.length() - 1) {
					...
					String key = line.substring(0, index); // Get first part,
															// e.g. "Host:"
					String value = line.substring(index + 1); // Get the rest,
																// e.g.
																// "www.rose-hulman.edu"

					// Lets strip off the white spaces from key if any and
